...
local function recursiveLoad(currentContext, data)
  function load(k,v)
  	assert(type(k)=="string" or type(k)=="number","i18n.load requires a string or number key but got "..type(k))

	table.insert(currentContext,k)
	local _type = type(v)

	if (_type == "string") then
		i18n.set(currentContext, v)
	elseif(_type =="table") then
		recursiveLoad(currentContext, v)
	else
		assert(false,"i18n.load requires("..k..") a string or table value but got ".._type)
	end
	currentContext[table.maxn(currentContext)]=nil
