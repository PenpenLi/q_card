...
display.p_center = ccp(display.cx,display.cy)
display.p_left = ccp(display.left,display.cy)
display.p_right = ccp(display.right,display.cy)
display.p_top = ccp(display.cx,display.top)
display.p_bottom = ccp(display.cx,display.bottom)
display.p_lt = ccp(display.left,display.top)
display.p_rt = ccp(display.right,display.top)
display.p_lb = ccp(display.left,display.bottom)
display.p_rb = ccp(display.right,display.bottom)
...
display.DefaultAramtureAnimName = "action1"

function display.newArmature(folder,resName,armatureName,animationName)
    local manager = CCArmatureDataManager:sharedArmatureDataManager()
    
--    manager:addArmatureFileInfo(folder.."/"..resName.."/"..resName..".png", folder.."/"..resName.."/"..resName..".plist", folder.."/"..resName.."/"..resName..".xml")
    echo(folder.."/"..resName.."/"..resName..".xml")
    manager:addArmatureFileInfo("","", folder.."/"..resName.."/"..resName..".xml")
    
    local armature = CCArmature:create(armatureName)
    armature:setAnchorPoint(ccp(0.5,0.5))

    local animation = armature:getAnimation()
    animation:setAnimationScale(30.0 / 60.0 ) 
--    if animationName ~= nil then
--      animation:play(animationName)
--    else
--      animation:play(display.DefaultAramtureAnimName)
--    end
    return armature
end
