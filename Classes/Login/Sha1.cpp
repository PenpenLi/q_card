...
std::string CSHA1::bytesToHexString(const unsigned char * input, size_t length) {
	std::string str;
...
std::string CSHA1::sha1(const std::string str){
	unsigned char p[20] = {0};
	CSHA1 *newCSHA1 = new CSHA1();
	newCSHA1->Reset();
	unsigned int len=str.length();
	const unsigned char* cstr= (const unsigned char*)str.c_str();
	newCSHA1->Update(cstr,len);
	newCSHA1->Final();
	newCSHA1->GetHash((unsigned char*)p);
	delete newCSHA1;
	return bytesToHexString(p,20);
}
NS_GAME_FRM_END
